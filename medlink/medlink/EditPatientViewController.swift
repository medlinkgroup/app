...
class EditPatientViewController: UIViewController, UIPickerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.id = editPatient._id
        self.doctorUID = editPatient.doctorUid
        loadData()
        createDatePicker()
        createObjetPicker()
    }
  ...
  func newInstance(detail: Patient) -> EditPatientViewController {
    ...
    @IBAction func save_btn(_ sender: Any) {
        ...
        geocoder.geocodeAddressString(place) { (placemarks, err) in
            ...
            self.patientService.edit(id: self.editPatient._id, firstName: firstName, lastName: lastName, phone: phone, photo: imageURL, email: email, doctorUid: self.doctorUID!, place: place, location: loc, birthDate: date, objetUid: self.selectedObjectId ?? "")
        ...
        if( objet != nil ){
                self.objetService.edit( id: objet._id , name: objet.name , isAttributed: false){ (success) in
                print(success)
                   }
            }
       
          let confirmationAlert = UIAlertController(title: NSLocalizedString("success", comment: "")
, message: NSLocalizedString("add_success", comment: "")
    ...
    func createDatePicker(){
       // datePicker.datePickerMode = .date
        let toolbar = UIToolbar()
         toolbar.sizeToFit()
        let doneButton = UIBarButtonItem(title: NSLocalizedString("done", comment: "")
, style: .plain, target: self, action: #selector(AddPatientViewController.donedatePicker ))
...
extension EditPatientViewController:  UIImagePickerControllerDelegate,
